<?php
/**
 * Class Agama_Breadcrumbs
 *
 * This file does the breadcrumbs handling for the vision framework.
 *
 * @author		ThemeVision
 * @copyright	(c) Copyright by ThemeVision
 * @link		http://theme-vision.com
 * @package 	VisionFramework
 * @since		Version 1.0
 */
...
class Agama_Breadcrumb {
	...
	private function __construct() {
		...
		$defaults = array(
			'home_prefix'            => esc_html( get_theme_mod( 'agama_breadcrum_prefix', '' ) ),
			'separator'              => esc_html( get_theme_mod( 'agama_breadcrum_separator', '/' ) ),
			'show_post_type_archive' => esc_attr( get_theme_mod( 'agama_breadcrumb_show_post_type_archive', false ) ),
			'show_terms'             => esc_attr( get_theme_mod( 'agama_breadcrumb_show_categories', true ) ),
			'home_label'             => esc_attr__( 'Home', 'agama-pro' ),
			'tag_archive_prefix'     => esc_attr__( 'Tag:', 'agama-pro' ),
			'search_prefix'          => esc_attr__( 'Search:', 'agama-pro' ),
			'error_prefix'           => esc_attr__( '404 - Page not Found', 'agama-pro' ),
		);
	...
	public static function get_instance() {
		if ( null === self::$instance ) {
			self::$instance = new Agama_Breadcrumb();
		}
	...
	private function prepare_breadcrumb_html() {
		...
		if ( is_singular() ) {
			...
			elseif ( is_404() ) {
				...
				if ( ( function_exists( 'tribe_is_event' ) && tribe_is_event() ) || Agama_Helper::is_events_archive() ) {
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = '<div class="vision-breadcrumbs">' . $this->html_markup . '</div>';
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = '<span class="vision-breadcrumb-prefix">' . $this->home_prefix . ':</span>';
			}
		}
	...
	private function get_breadcrumb_home() {
		...
		if ( ! is_front_page() ) {
			...
			$home_link = $this->get_single_breadcrumb_markup( __( 'Home', 'agama-pro' ), false, false );
		}
	...
	private function get_post_terms() {
		...
		elseif ( 'agama_portfolio' == $this->post->post_type || 'vision_portfolio' == $this->post->post_type || $portfolio_slug == $this->post->post_type ) {
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true ) {
		...
		if( $microdata && get_theme_mod( 'agama_rich_snippets', true ) ) {
		...
		if ( $separator ) {
			$separator_markup = '<span class="vision-breadcrumb-sep">' . $this->separator . '</span>';
		}
